    ...
    <form onSubmit={handleSave} className="space-y-4 mt-4 text-xs">
      ...
      {!isAlreadyMuted ? (
        ...
        <div className="bg-amber-50 dark:bg-amber-950/40 p-3 rounded-lg border border-amber-200 dark:border-amber-800 text-amber-900 dark:text-amber-300 space-y-1">
          ...
          <div>Reason: <strong>{finding.muteRecord?.reason}</strong></div>
          {finding.muteRecord?.justification && (
            <div>Justification: <em>"{finding.muteRecord.justification}"</em></div>
          )}
          <div className="text-[10px] text-amber-600 dark:text-amber-400 mt-2">
            Muted by {finding.muteRecord?.mutedBy || 'Reviewer'} on {new Date(finding.muteRecord?.mutedAt || '').toLocaleString()}
          </div>
        </div>
      )}
